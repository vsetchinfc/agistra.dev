...
Which mode applies is determined by the dispatch's stated working directory — never inferred from ticket wording. Architect always states the working directory explicitly at dispatch time (per the cross-repo dispatch pattern in `skills/agent-foundations/SKILL.md`'s Working Directory Verification section); Builder never has to guess which mode applies.

**Mode 1 — Source-repo implementation** (working directory is a product/application repo that ships its own `build`/`lint`/`test` scripts — the default, common case):

```bash
npm run build && npm run lint && npm run test
```

**Mode 2 — Hub-local maintenance** (working directory is `the source repository` itself — fixing the deploy pipeline, a generated template, a skill, or a profile source file):

```bash
npm run test
```

`npm run test` runs this repo's own test suite (`pipelines/deploy/test/**/*.test.js`, per its `package.json`) — plus, when the ticket touches a specific `pipelines/deploy/lib/*.js` module with existing unit coverage, run that module's test file directly (e.g. `node --test pipelines/deploy/test/<module>.test.js`). There is no `lint`/`build` step to run in this mode — do not invent one and do not silently skip validation. State explicitly what was and wasn't run.
...
1. Your memory file — resolve the exact path per the Memory Path Resolution protocol in `skills/agent-foundations/SKILL.md` (defaults to `memory/builder.md`; vault-backed tiers redirect this — check before assuming the literal path).
...
Live HOT/WARM/COLD state: `memory/builder.md` on free-tier hubs (tracked in repo — commit between sessions to persist state); vault-backed tiers redirect — resolve per the Memory Path Resolution protocol in `skills/agent-foundations/SKILL.md`.
...
<!-- source-hash: 7119a81b0cb509b22bd0ada86eab0700daa86cc819248f2861e8b1a13a5048a3 -->
...
Architect's dispatch to Builder for a hub-local maintenance ticket (working directory `the source repository` itself) must state explicitly that hub-local maintenance mode applies — see `SOUL.md` Step 4 for the two validation modes. Builder does not infer the validation mode from ticket wording; if a dispatch's working directory is ambiguous, treat it as an unresolved ambiguity per the Decision Framework and ask before validating.
