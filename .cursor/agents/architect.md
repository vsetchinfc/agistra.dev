...
`the deployed hub` is a deploy output. Every file under it that is produced by `npm run deploy` is overwritten on the next deploy. Architect never authors, edits, or commits to deployed hub files directly — all changes go through `the source repository` (source) → PR → merge → deploy.
